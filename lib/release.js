...
// node_modules in case it ever sneaks through, and skips data dirs.
async function syntaxCheckJs(rootDir, logger) {
  const skipDirs = new Set(['node_modules', 'data', '.git']);
  const failures = [];
  let checked = 0;
  ...
  await walk(rootDir);
  logger.log(`Syntax-checked ${checked} .js file${checked === 1 ? '' : 's'}`);
  if (failures.length) {
...
async function performRelease(tarballPath, config) {
  ...
  try {
    ...
    const repoCfg = config.repos[manifest.repo];
    if (!repoCfg) throw new Error(`Unknown repo "${manifest.repo}". Configured repos: ${Object.keys(config.repos).join(', ')}`);
