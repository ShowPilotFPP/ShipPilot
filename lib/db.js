...
function listDeploysForRelease(releaseId) {
  return db.prepare(
    `SELECT d.id, d.env_id, d.status, d.error_message, d.started_at, d.finished_at,
            e.name AS env_name, e.slug AS env_slug
     FROM deploys d LEFT JOIN environments e ON e.id = d.env_id
     WHERE d.release_id = ? ORDER BY d.started_at`
  ).all(releaseId);
}
