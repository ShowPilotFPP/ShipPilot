...
function load() {
  ...
  // Per-environment SSH keys (separate from repo keys so a leaked deploy
  // key doesn't grant access to GitHub repos and vice versa).
  if (!cfg.envSshKeysDir) cfg.envSshKeysDir = path.resolve(root, 'data/env-ssh-keys');
  else if (!path.isAbsolute(cfg.envSshKeysDir)) cfg.envSshKeysDir = path.resolve(root, cfg.envSshKeysDir);
  // Pinned host keys for environment SSH connections (TOFU).
  if (!cfg.envKnownHostsDir) cfg.envKnownHostsDir = path.resolve(root, 'data/env-known-hosts');
  else if (!path.isAbsolute(cfg.envKnownHostsDir)) cfg.envKnownHostsDir = path.resolve(root, cfg.envKnownHostsDir);
